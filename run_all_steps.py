...
------------------------------------------------------------
Input:
    - ./src_data/src_filtered_stid_under30m_b.csv
...
Output (all prefixed with "step1_"):
    - step1_stid_depth_adjusted_env.csv
        * Depth-adjusted environmental panel at station level (monthly)
...
Note:
    ...
    The CSV file is written mainly as a backup/log.

------------------------------------------------------------
STEP 2: Env–CB analysis (stid/admin-level, quarterly & yearly)
------------------------------------------------------------
Main external input:
    - ./src_data/src_coralline_covers_col.csv
        * Admin-level CB area (normal/progressive/critical) by year

Key intermediate objects (kept in memory, NOT saved by default):
    - stid_slope_summary
        * Station-level env slopes (quarterly time index)
    - admin_env_q
        * Admin-level quarterly env panel (weighted by station reliability)
    - admin_env_slope
        * Admin-level env trend slopes
    - admin_env_trend_summary
        * Categorical env trend (increasing/decreasing/no_change, significance)
    - admin_cb_slope
        * Admin-level CB (CB_1, CB_2, CB_3) trend slopes
    - admin_CB_env_long
        * Joined table of admin-level env slopes and CB slopes
    - admin_CB_env_cor_admin
        * Within-admin env–CB time-series correlation (per env × CB type)

STEP 2 CSV outputs (all prefixed with "step2_"):
    - step2_tab2_admin_CB3_slope_summary.csv
        * Admin-level trend summary for CB_3 (critical CB area)
        * Columns include cb_slope, cb_p_value, cb_r^2, etc.
    - step2_CB3_env_correlation_table.csv
        * Wide matrix of CB_3 vs environmental-variable correlations
        * Index: (region, admin), columns: env variables, values: cor_ts

STEP 2 plots / diagnostics:
    - A diagnostic CB3–environment correlation heatmap
        * Shown with plt.show() (not saved to file)
        * Based on cb3_matrix (CB_3 vs env correlation matrix)

Disabled outputs (commented out in code; not produced by default):
    ...
    - Advanced CB3 ranking/driver tables (controlled by SAVE_ADVANCED_CB3_TABLES)

------------------------------------------------------------
STEP 3: Report tables & plots (manuscript-ready summaries)
------------------------------------------------------------
Inputs (all passed in memory from STEP 2):
    - env_trend_core            (admin_env_trend_summary)
    - cb_slope                  (admin_cb_slope)
    - cb_ts_corr                (admin_CB_env_cor_admin)
    - admin_cb_env_long         (admin_CB_env_long)
    - cb3_matrix                (CB3 vs env correlation matrix)

STEP 3 CSV outputs (no step3_ prefix in filenames):
    - env_trend_slope_matrix.csv
        * Wide matrix of admin-level env slopes
        * Index: (region, admin), columns: raw_variable
    - env_CB3_combined_table.csv
        * Integrated table of env trends, CB3 trends, and env–CB3 correlations
    - region_env_slope_summary.csv
        * Region-level mean slopes of environmental variables

STEP 3 figure outputs (PNG):
    - fig3_Normalized_slopes_of_env._variables.png
        * Heatmap of sign-preserving normalized env slopes by municipality
    - fig4_Trends_of_CB3_by_municipality_2014-2024.png
        * Barplot of CB3 area trends by municipality (2014–2024)
    - fig5_Relationships_between_environmental_variables_and_CB3_change.png
        * Multi-panel scatter/regression: env_slope vs CB3 slope
    - fig6_Environment-ecology_decoupling_matrix_for_temperature.png
        * 2×2 decoupling matrix plot (temperature vs CB3 slope, annotated)
    - fig7_Heatmap_for_municipality-env._variable_in_CB3.png
        * Report-version heatmap of CB3 vs env correlations by municipality
    - fig8_Integrated_trend_matrix_of_environmental_change_and_CB3_intensification.png
        * Categorical matrix of combined env and CB3 trends
    - Regional_mean_slopes_of_environmental_variables.png
        * Barplot of regional mean env trends
...
# ======================================================================
# (Optional) Change working directory to the script location
# ======================================================================
try:
    THIS_DIR = os.path.dirname(os.path.abspath(__file__))
    os.chdir(THIS_DIR)
except NameError:
    # e.g., Jupyter environment
    pass
...
# ------------------------------------------------------------
env_path = "./src_data/src_filtered_stid_under30m_b.csv"
...
print(f"[STEP1] {output_path} saved.")
...
# ===================================================================
# 4. Admin-level CB trends (src_coralline_covers_col.csv)
# ===================================================================

cb_panel_path = "./src_data/src_coralline_covers_col.csv"
...
if not all(col in cb_panel.columns for col in required_cb_cols):
    raise ValueError(
        "./src_data/src_coralline_covers_col.csv must contain: "
...
admin_cb_slope_path = step2_prefix + "tab2_admin_CB3_slope_summary.csv"
admin_cb_slope.to_csv(
    admin_cb_slope_path,
    index=False,
    float_format=CSV_FLOAT_FORMAT
)
print(f"[STEP2] {admin_cb_slope_path} saved.")
...
if cb3.empty:
    ...
    cb3_matrix = cb3.pivot(
        index=["region", "admin"],
        columns="variable",
        values="cor_ts"
    )

    # Keep zero correlations as they are; only use real NaNs for row dropping
    # Drop only admins whose entire row is NaN (no data at all)
    cb3_matrix = cb3_matrix.dropna(how="all", axis=0)

    if cb3_matrix.empty:
        print("Warning: All CB3 correlations are zero/NaN; no table/heatmap created.")
    else:
        ...
        print(f"[STEP2] {cb3_table_path} saved.")
        
        # Heatmap (STEP 2 diagnostic version, displayed only)
        ...
        ax.set_title(
            "Correlation between CB and Environmental Variables",
            fontsize=18,
            pad=20
        )
        ...
        cbar.set_label("Correlation (CB vs Env)", fontsize=14)
...
print(f"[STEP3] {env_slope_matrix_csv_path} saved.")

# 2-A. Sign-preserving normalized slope heatmap
if not env_slope_matrix.empty:
    # 1) Sign-preserving normalization per variable: slope / max(|slope|)
    env_slope_norm = env_slope_matrix.copy().astype(float)

    for col in env_slope_norm.columns:
        col_vals = env_slope_norm[col].values.astype(float)
        max_abs = np.nanmax(np.abs(col_vals))
        if np.isnan(max_abs) or max_abs == 0:
            # If all values are 0 or NaN, leave the column as 0
            env_slope_norm[col] = 0.0
        else:
            env_slope_norm[col] = env_slope_norm[col] / max_abs
    ...
    ax = sns.heatmap(
        ...
        linewidths=0.5,
        linecolor="gray",
        cbar_kws={"label": "Normalized slope (sign-preserving, per variable)"}
    )

    ax.set_xlabel("Environmental variables", fontsize=14)
    ax.set_ylabel("Region / Admin", fontsize=14)
    ax.set_title(
        "Normalized slopes of depth-adjusted environmental variables\n"
        "by municipality (2014–2024)",
        fontsize=16,
        pad=20
    )
    ...
    env_slope_heatmap_norm_path = (
        step3_prefix + "fig3_Normalized_slopes_of_env._variables.png"
    )
    plt.savefig(env_slope_heatmap_norm_path, dpi=300)
    plt.close()
    print(f"[STEP3] {env_slope_heatmap_norm_path} saved.")
else:
    print("Warning: No data for normalized env_slope_matrix heatmap.")
    
"""
# 2-B. Raw slope heatmap (style unified)
...
"""
...
if "cb3_matrix" not in locals():
    ...
    # MOD: mask out zero correlations in the plot (keep them as NaN so they are not colored)
    cb3_plot = cb3_plot.replace(0, np.nan)
    ...
    if not cb3_plot.empty:
        ...
        ax.set_xlabel("Environmental variables", fontsize=14)
        ax.set_ylabel("Region / Admin", fontsize=14)
        ax.set_title("Heatmap for municipality-environmental variable slope",
                     fontsize=18, pad=20)
        ...
        cb3_heatmap_path = step3_prefix + "fig7_Heatmap_for_municipality-env._variable_in_CB3.png"
        plt.savefig(cb3_heatmap_path, dpi=300)
        plt.close()
        print(f"[STEP3] {cb3_heatmap_path} saved.")
    else:
...
print(f"[STEP3] {combined_out_path} saved.")
...
if cb3_plot_df.empty:
    print("Warning: No valid CB_3 rows (region/admin/cb_slope). Figure 4 will not be generated.")
else:
    ...
    ax.set_title("Trends of critical-stage coralline barren (CB3) area by municipality, 2014–2024",
                 fontsize=16, pad=16)
    ...
    fig7_path = step3_prefix + "fig4_Trends_of_CB3_by_municipality_2014-2024.png"
    plt.savefig(fig7_path, dpi=300)
    plt.close()
    print(f"[STEP3] {fig7_path} saved.")


# -------------------------------------------------------------------
# Figure: Integrated Env–CB3 trend matrix (heatmap, categorical colors)
# -------------------------------------------------------------------
...
if combined_out is None or combined_out.empty:
    ...
    if plot_df.empty:
        ...
        # Font style unified with the Z-score heatmap
        ax.set_xlabel("Environmental variables", fontsize=14)
        ax.set_ylabel("Region / Admin", fontsize=14)
        ax.set_title(
            "Integrated trend matrix of environmental change and CB3 intensification",
            fontsize=18,
            pad=20
        )
        ...
        fig8_path = step3_prefix + "fig8_Integrated_trend_matrix_of_environmental_change_and_CB3_intensification.png"
        plt.savefig(fig8_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"[STEP3] {fig8_path} saved.")
...
print(f"[STEP3] {region_group_csv_path} saved.")
...
if not region_group.empty:
    ...
    region_bar_path = step3_prefix + "Regional_mean_slopes_of_environmental_variables.png"
    plt.savefig(region_bar_path, dpi=300)
    plt.close()
    print(f"[STEP3] {region_bar_path} saved.")
else:
...
if not cb3_slope_long.empty:
    ...
    fig.suptitle(
        "Relationships between environmental variables and CB3 change across municipalities",
        fontsize=16,
        y=0.99
    )
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    scatter_path = step3_prefix + "fig5_Relationships_between_environmental_variables_and_CB3_change.png"
    plt.savefig(scatter_path, dpi=300)
    plt.close()
    print(f"[STEP3] {scatter_path} saved.")
...
# Note: you can move this import to the top of the script if preferred
...
if df_matrix.empty:
    ...
    # Legend and axis settings remain as in the original version
    ax.legend(
        title="Region",
        loc="upper left",
    ...
    # ---------------------------------------------------------
    # Modified section: apply text-overlap avoidance algorithm
    # ---------------------------------------------------------
    texts = []  # List to store text objects

    for i, row in df_matrix.iterrows():
        # Add text objects to the list instead of drawing directly
    ...
    # adjust_text automatically optimizes label positions
    # arrowprops: draws a line from labels to points when separated
    adjust_text(
        texts,
        ax=ax,
        arrowprops=dict(arrowstyle='-', color='gray', lw=0.5),
        only_move={'points': 'y', 'text': 'xy'},  # allow labels to move to reduce overlap
        lim=1000  # iteration limit for optimization
    )
    # ---------------------------------------------------------

    # Quadrant annotations and remaining settings follow the original logic
    ...
    ax.set_title(
        f"Environment-Ecology Decoupling Matrix for temperature\n(Variable: {target_var})",
        fontsize=10,
        pad=20
    )
    ...
    matrix_plot_path = step3_prefix + "fig6_Environment-ecology_decoupling_matrix_for_temperature.png"
    plt.savefig(matrix_plot_path, dpi=300)
    plt.close()
    print(f"[STEP3] {matrix_plot_path} saved.")
